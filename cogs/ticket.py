...
from utils.cooldown import ticket_cooldown

_TICKET_COOLDOWN_SURE = 30.0  # saniye
...
class TicketDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        # Cooldown kontrolü
        kalan = ticket_cooldown.kontrol(f"ticket:{interaction.user.id}", _TICKET_COOLDOWN_SURE)
        if kalan is not None:
            await interaction.response.send_message(
                f"<a:no:1478524993670479942>  Çok hızlı! **{kalan:.0f} saniye** bekle.",
                ephemeral=True,
            )
            return
